...
interface IntroRoundAnswerScreenProps {
  game: any;
  gameId: Id<"games">;
}

export default function IntroRoundAnswerScreen({ game, gameId }: IntroRoundAnswerScreenProps) {
  const [showAnswer, setShowAnswer] = useState(false);
  const updateGamePhase = useMutation(api.games.updateGamePhase);

  const handleDistributePoints = async () => {
    await updateGamePhase({ gameId, phase: "intro_round_jury" });
  };
