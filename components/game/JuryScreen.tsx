...
export default function JuryScreen({ game, gameId }: JuryScreenProps) {
  ...
  const getRewardPoints = () => {
    if (!game.currentQuestion) return 5;
    switch (game.currentQuestion.difficulty) {
      case "diep": return 5;
      case "dieper": return 10;
      case "diepst": return 20;
      default: return 5;
    }
  };

  const getAvailablePoints = () => {
    return (game.players.length - 1) * 5;
  };
  ...
  const handleFinish = async () => {
    ...
    await updateGamePhase({ gameId, phase: "scoreboard" });
  };
